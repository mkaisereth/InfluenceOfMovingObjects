function statisticalAnalysis_alt2(staticFrameData, dynamicFrameData, stepDepthsLeft, stepDepthsMiddle, stepDepthsRight)
...
%                     plot([static.closer.median+(static.closer.range/2), ...
%                         static.closer.median-(static.closer.range/2)], ...
%                         [0, 0], 'xg', 'LineWidth', 5, 'MarkerSize', 5)
%                     legend({'Histogram', 'Median', 'range Bound'});
...
%                     plot([static.further.median+(static.further.range/2), ...
%                         static.further.median-(static.further.range/2)], ...
%                         [0, 0], 'xg', 'LineWidth', 5, 'MarkerSize', 5)
%                     legend({'Histogram', 'Median', 'range Bound'});
...
%                         plot([dynamic.closer.median+(dynamic.closer.range/2), ...
%                             dynamic.closer.median-(dynamic.closer.range/2)], ...
%                             [0, 0], 'xg', 'LineWidth', 5, 'MarkerSize', 5)
%                         legend({'Histogram', 'Median', 'range Bound'});
%                         title('Closer Step Distribution');
%                         xlabel('Z-Axis Distance Distribution [mm]');
...
%                         plot([dynamic.further.median+(dynamic.further.range/2), ...
%                             dynamic.further.median-(dynamic.further.range/2)], ...
%                             [0, 0], 'xg', 'LineWidth', 5, 'MarkerSize', 5)
%                         legend({'Histogram', 'Median', 'range Bound'});
%                         title('Further Step Distribution');
%                         xlabel('Z-Axis Distance Distribution [mm]');
%                         ylabel('Point Cloud Count [-]');
                    ...
                    if (stepNum == 1 && stepROI ~= 3) || (stepNum == 10 && stepROI == 3) || (strcmp(stepSide{:}, 'left') && stepROI == 4) || (strcmp(stepSide{:}, 'right') && stepROI == 2)
                        results(frameNum).rangeErr.(stepSide{:})(stepNum, stepROI) = ...
                        ...
                        results(frameNum).staticFurtherSteps.(stepSide{:})(stepNum, stepROI) = ...
                            {0};
                        results(frameNum).staticRange.(stepSide{:})(stepNum, stepROI) = ...
                            {[static.range, 0]};
                        ...
                        results(frameNum).dynamicFurtherSteps.(stepSide{:})(stepNum, stepROI) = ...
                            {0};
                        results(frameNum).dynamicRange.(stepSide{:})(stepNum, stepROI) = ...
                            {[dynamic.range, 0]};
...
%                         ax.FontWeight = 'bold';
    ...
    meanrangeErr = mean([mean(nonzeros([results(1).rangeErr.left{:}])), mean(nonzeros([results(2).rangeErr.left{:}])), mean(nonzeros([results(3).rangeErr.left{:}])), ...
        mean(nonzeros([results(1).rangeErr.right{:}])), mean(nonzeros([results(2).rangeErr.right{:}])), mean(nonzeros([results(3).rangeErr.right{:}]))]);
    maxrangeErr = max([max(nonzeros([results(1).rangeErr.left{:}])), max(nonzeros([results(2).rangeErr.left{:}])), max(nonzeros([results(3).rangeErr.left{:}])), ...
        max(nonzeros([results(1).rangeErr.right{:}])), max(nonzeros([results(2).rangeErr.right{:}])), max(nonzeros([results(3).rangeErr.right{:}]))]);
    ...
    fprintf('\t\tMean range difference: \t%fmm\n', meanRangeErr);
    fprintf('\t\tMax range difference: \t%fmm\n', maxRangeErr);
